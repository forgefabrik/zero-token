...
const XIAOMIMO_URL = "https://xiaomimo.com";
const AUTH_PATTERNS = ["/auth/", "/login", "/signin"];
const COOKIE_NAMES = ["xiaomimo", "session", "token", "Authorization"];
const TOKEN_KEYS = ["accessToken", "mimo-token", "userToken"];

/**
 * XiaoMiMo-Web-Auth: Browser öffnen, auf manuellen Login warten,
...
export const auth: ProviderAuthFunction = async (config) => {
  ...
  try {
    ...
    await waitForUrlLogin(page, XIAOMIMO_URL, AUTH_PATTERNS, config.loginTimeout ?? 300_000);
